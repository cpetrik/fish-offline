%%%% File naming system
function [fname,simname] = sub_fname_dple_exper_jhub(param,exper)
...
Sdir = '/glade/work/cpetrik/fish-offline/feisty_output/';
if (~isfolder([Sdir,simname]))
    mkdir([Sdir,simname])
end

%! Setup netcdf path to store to
if (frate==0)
    fname = [Sdir,simname, '/DPLE_',exper,'pristine'];
elseif (param.Jsel~=0.1)
    fname = [Sdir,simname, '/DPLE_',exper,sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = [Sdir,simname, '/DPLE_',exper,'fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = [Sdir,simname, '/DPLE_',exper,sel,'_fish',tfish(2:end)];
end
