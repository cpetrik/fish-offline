...
%tfish = {'S','M','L','F','P','D','A','B'};
tfish = {'B','D','A','L','P','F','M','S'};
tyr = {'0','1','2','3','4','5'};
...
save([fpath,'FOSI_biom_regress_drivers_div2SD.mat'],'tanom','tfish',...
    'tyr','yr');
...
for L = 1:length(lid)
    ...
    Cmat = nan*ones(8,length(yr),5);
    Pmat = nan*ones(8,length(yr),5);
    ...
    for j = 1:5 %forcing vars
        ...
        for k=1:length(yr) %Linear regression at diff lags
            ...
            Cmat(8,k,j) = mdl0.Coefficients.Estimate(2);
            Pmat(8,k,j) = mdl0.Coefficients.pValue(2);
            clear mdl0
    ...
    eval(['save(''' [fpath 'FOSI_biom_regress_drivers_div2SD.mat'] ''',''' ['Cmat_' ilme] ''',''-append'')']);
    eval(['save(''' [fpath 'FOSI_biom_regress_drivers_div2SD.mat'] ''',''' ['Pmat_' ilme] ''',''-append'')']);
