% Make mat files of interpolated time series from CESM HR (JRA)
% Total mort 
% Lg zoo calc from frac diat +0.5*coccos out of all 4 pythos
% Add last mon y-1 and 1st mo y+1 to interp
% Just Cal Curr region
...
load([fpath 'g.e22.G1850ECO_JRA_HR.TL319_t13.004.FIESTY-forcing_hiresJRA_CAcurr.mat'],...
    'FillValue','missing_value','TEMP_100m','TEMP_100m_units','TEMP_bottom',...
...
load([fpath 'g.e22.G1850ECO_JRA_HR.TL319_t13.004.FIESTY-forcing_hiresJRA_CAcurr_meszoo_totloss_allphytoC.mat'],...
    'LzooC_100m','Lzoo_loss_100m');
%load([fpath 'gridspec_POP_gx1v6_noSeas.mat'],'mask');

%% doubles, nans, & zeros
TEMP_100m = double(TEMP_100m);
...
LzooC_100m(TEMP_bottom >= 9.9e+36) = nan;
Lzoo_loss_100m(TEMP_bottom >= 9.9e+36) = nan;

LzooC_100m(LzooC_100m<0) = 0.0;
Lzoo_loss_100m(Lzoo_loss_100m<0) = 0.0;
POC_FLUX_IN_bottom(POC_FLUX_IN_bottom<0) = 0.0;

%% Units
%poc flux: mmol/m^3 cm/s
%zoo loss: mmol/m^3 cm/s
%zoo: mmolC/m^3 cm
%tp: degC
%tb: degC
...
nyrs = mos/12;
%yrs = 1:nyrs;
yrs = floor(yr(1:12:end));

Tdays=1:365;

%% test that all same orientation
test1 = squeeze(double(TEMP_100m(:,:,200)));
test2 = squeeze(double(TEMP_bottom(:,:,200)));
test3 = squeeze(double(LzooC_100m(:,:,200)));
test4 = squeeze(double(Lzoo_loss_100m(:,:,200)));
...
for y = 1:nyrs
    ...
    [ni,nj,nt] = size(LzooC_100m);
    mask = squeeze(LzooC_100m(:,:,1));  % no mask for this grid yet
    ...
    for j = 1:NID
        ...
        X = squeeze(Tp(m,n,:));
        xi = interp1(Time, X, Tdays,'linear','extrap');
        D_Tp(j,:) = xi;
        ...
        X = squeeze(dZm(m,n,:));
        xi = interp1(Time, X, Tdays,'linear','extrap');
        D_dZm(j,:) = xi * 1e-9 * 1e4 * 12.01 * 9.0 * 60 * 60 * 24;
        ...
        X = squeeze(det(m,n,:));
        xi = interp1(Time, X, Tdays,'linear','extrap');
        D_det(j,:) = xi * 1e-9 * 1e4 * 12.01 * 9.0 * 60 * 60 * 24;
...
%     D_det(isnan(D_det)) = 0.0;
    ...
    % save
    save([fpath 'Data_cesm_hr_CCS_daily_',num2str(YR),'.mat'], 'ESM');
