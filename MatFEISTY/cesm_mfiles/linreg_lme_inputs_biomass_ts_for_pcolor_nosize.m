...
for L = 1:length(lid)
    ...
    Cmat = nan*ones(5,length(yr),5);
    Pmat = nan*ones(5,length(yr),5);
    ...
    eval(['save(''' [fpath 'FOSI_biom_regress_drivers_div2SD.mat'] ''',''' ['Cmat_nosize_' ilme] ''',''-append'')']);
    eval(['save(''' [fpath 'FOSI_biom_regress_drivers_div2SD.mat'] ''',''' ['Pmat_nosize_' ilme] ''',''-append'')']);
