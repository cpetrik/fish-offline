...
for L = 1:length(lid) %LME
    ...
    for j = 1:length(tanom) %drivers
        ...
        for k=1:length(yr) %Linear regression at diff lags
            ...
            %Fish
            n = n+1;
            [rp,pp] = corrcoef(sclim , (arf(L,yst+t:yen))');
            FtabC(k) = rp(1,2);
            FtabP(k) = pp(1,2);
            clear rp pp

            n = n+1;
            [rp,pp] = corrcoef(sclim , (arp(L,yst+t:yen))');
            PtabC(k) = rp(1,2);
            PtabP(k) = pp(1,2);
            clear rp pp

            n = n+1;
            [rp,pp] = corrcoef(sclim , (ard(L,yst+t:yen))');
            DtabC(k) = rp(1,2);
            DtabP(k) = pp(1,2);
            clear rp pp

            n = n+1;
            [rp,pp] = corrcoef(sclim , (ara(L,yst+t:yen))');
            AtabC(k) = rp(1,2);
            AtabP(k) = pp(1,2);
            clear rp pp
...
writetable(Ftab,[rpath,'LME_sig_corr_maxlag_driver_recruit_F.csv'],...
    'Delimiter',',');
writetable(Ptab,[rpath,'LME_sig_corr_maxlag_driver_recruit_P.csv'],...
    'Delimiter',',');
writetable(Dtab,[rpath,'LME_sig_corr_maxlag_driver_recruit_D.csv'],...
    'Delimiter',',');
writetable(Atab,[rpath,'LME_sig_corr_maxlag_driver_recruit_A.csv'],...
    'Delimiter',',');

save([rpath,'LME_sig_corr_maxlag_driver_recruit.mat'],'lid',...
