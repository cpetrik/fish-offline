...
fcoef <- read.csv(paste0(datar,"LME_corr_maxlag_driver_nu_F.csv"),sep=",",header = T,stringsAsFactors = F)
pcoef <- read.csv(paste0(datar,"LME_corr_maxlag_driver_nu_P.csv"),sep=",",header = T,stringsAsFactors = F)
dcoef <- read.csv(paste0(datar,"LME_corr_maxlag_driver_nu_D.csv"),sep=",",header = T,stringsAsFactors = F)
acoef <- read.csv(paste0(datar,"LME_corr_maxlag_driver_nu_A.csv"),sep=",",header = T,stringsAsFactors = F)
...
png(paste0(figp,"heatmap_LME_driver_prod_A_corr_coeffs_5clus.png"),    # create PNG for the heat map        
...
fdend_6 <- cutree(fdfish, k=5)
fcoef50$Cluster <- fdend_6
# reduce the size of the labels:
fdfish <- set(fdfish, "labels_cex", 0.8)

png(paste0(figp,"heatmap_LME_driver_prod_F_corr_coeffs_5clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
png(paste0(figp,"heatmap_LME_driver_prod_P_corr_coeffs_4clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
png(paste0(figp,"heatmap_LME_driver_prod_D_corr_coeffs_4clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
### Save clustering
write.table(acoef,paste0(datar,"LME_driver_prod_A_corr_coeffs_5cluster.csv"),sep=",",row.names=F)
write.table(fcoef,paste0(datar,"LME_driver_prod_F_corr_coeffs_5cluster.csv"),sep=",",row.names=F)
write.table(pcoef,paste0(datar,"LME_driver_prod_P_corr_coeffs_4cluster.csv"),sep=",",row.names=F)
write.table(dcoef,paste0(datar,"LME_driver_prod_D_corr_coeffs_4cluster.csv"),sep=",",row.names=F)
