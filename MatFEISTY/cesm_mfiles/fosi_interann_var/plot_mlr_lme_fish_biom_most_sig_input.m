...
load([spath,'LMEs_mlr_drivers_ALLdiv2SD_maxcoefs.mat'])
...
title('All fishes')
%lgd = legend(ctex,'Location','southeast');
print('-dpng',[ppath 'Bar_LMEs_driver_mlr_maxcoef_allfish.png'])
...
for i=1:length(lid)
    L=lid(i);
    if (LBtab(i,2) <= 0.05)
        b=bar(L,LBtab(i,1),'EdgeColor',mcol(LBtab(i,3),:),'FaceColor',mcol(LBtab(i,3),:));
        hold on
    else
        b=bar(L,LBtab(i,1),'EdgeColor',[1 1 1],'FaceColor',[1 1 1]);
        hold on
    end
...
print('-dpng',[ppath 'Bar_LMEs_driver_mlr_maxcoef_fntypes.png'])
...
title('All fishes')
print('-dpng',[ppath 'Map_LMEs_driver_mlr_maxcoef_allfish.png'])
...
colormap(mcol)
colorbar('Position',[0.7 0.2 0.03 0.25],'Ticks',1:8,'TickLabels',ctex)
title('Benthos')
print('-dpng',[ppath 'Map_LMEs_driver_mlr_maxcoef_fntypes.png'])
