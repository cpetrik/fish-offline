...
load([spath 'LME_alltypes_mlr_coeffs_ALLdiv2SD_alllags.mat'])
...
for L = 1:length(lid)
    ...
    %% Benthos
    %Det
    maxC = max(abs(Bcoef(i,did)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,1) = Bcoef(i,fid);
    end
    clear maxC
    %TB
    maxC = max(abs(Bcoef(i,bid)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,2) = Bcoef(i,fid);
    end
    clear maxC
    %TP
    maxC = max(abs(Bcoef(i,pid)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,3) = Bcoef(i,fid);
    end
    clear maxC
    %Zmeso
    maxC = max(abs(Bcoef(i,zid)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,4) = Bcoef(i,fid);
    end
    clear maxC
    %ZmLoss
    maxC = max(abs(Bcoef(i,mid)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,5) = Bcoef(i,fid);
    end
    clear maxC
    %Det:TB
    maxC = max(abs(Bcoef(i,dtb)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,6) = Bcoef(i,fid);
    end
    clear maxC
    %Zmeso:TP
    maxC = max(abs(Bcoef(i,ztp)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,7) = Bcoef(i,fid);
    end
    clear maxC
    %ZmLoss:TP
    maxC = max(abs(Bcoef(i,mtp)));
    if(~isnan(maxC))
        fid = find(abs(Bcoef(i,:))==maxC);
        LBtab(L,8) = Bcoef(i,fid);
    end
    clear maxC
...
%%
writetable(Ftab1,[spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc_F.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Ptab1,[spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc_P.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Dtab1,[spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc_D.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Atab1,[spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc_A.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Btab1,[spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc_B.csv'],...
    'Delimiter',',','WriteRowNames',true);

save([spath,'LMEs_mlr_drivers_ALLdiv2SD_alllags_reduc.mat'],...
