...
% per year also
Lzoo_loss_year = 365 * Lzoo_loss_150m;
POC_FLUX_year = 365 * POC_FLUX_IN_bottom;
...
zlosy = nan*ones(ni,nj,nyr);
dety = nan*ones(ni,nj,nyr);
for n=1:nyr
    ...
    zlosy(:,:,n)=nanmean(Lzoo_loss_year(:,:,st(n):en(n)),3);
...
xzly = NaN*ones(size(mzly));
xdety = NaN*ones(size(det2y));
...
azlosy = xzly - nanmean(xzly,2);
adety = xdety - nanmean(xdety,2);
...
Azlosy = nan*ones(ni,nj,nyr);
Adety  = nan*ones(ni,nj,nyr);
...
mzly_mean = nanmean(zlosy,3);
dety_mean = nanmean(dety,3);
...
cvzlosy = zlosy_std ./ mzly_mean;
cvdety = dety_std ./ dety_mean;
...
save([fpath 'CESM_FOSI_v15_interann_var_forcings.mat'],...
    'tp_mean','tb_mean','det_mean','mz_mean','mzl_mean','dety_mean','mzly_mean',...
    'tp_std','tb_std','det_std','zoo_std','zlos_std','dety_std','zlosy_std',...
    'tp_stda','tb_stda','det_stda','zoo_stda','zlos_stda','dety_stda','zlosy_stda',...
    'cvtp','cvtb','cvdet','cvzoo','cvzlos','cvdety','cvzlosy');
