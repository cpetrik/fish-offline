...
%% All corrs
CFtab = nan*ones(63,9,5);
PFtab = nan*ones(63,9,5);
CPtab = CFtab;
PPtab = CFtab;
CDtab = CFtab;
PDtab = CFtab;
...
%% obsfish feisty vars
load([spath 'LMEs_corr_catch_satyrs_obsfish_lags.mat'])
ftex = tanom;

%sat
CAtab(:,7:9,1:4) = AtabC(:,1:3,:);
CFtab(:,7:9,1:4) = FtabC(:,1:3,:);
CPtab(:,7:9,1:4) = PtabC(:,1:3,:);
CDtab(:,7:9,1:4) = DtabC(:,1:3,:);

PAtab(:,7:9,1:4) = AtabP(:,1:3,:);
PFtab(:,7:9,1:4) = FtabP(:,1:3,:);
PPtab(:,7:9,1:4) = PtabP(:,1:3,:);
PDtab(:,7:9,1:4) = DtabP(:,1:3,:);

clear AtabC AtabP FtabC FtabP PtabC PtabP DtabC DtabP tanom

%% Corrs w/effort
load([spath,'LMEs_corr_catch_satyrs_effort_nolags.mat'])
...
%%
tanom = {'TP','TB','Det','ZmLoss','SST','chl','Biom','Prod','Catch','Effort'};
...
for L = 1:length(lid)
    ...
    %% force prey & fish corrs to be pos or zero (3,4,6,7,8,9,10)
    AtabC(3,AtabC(3,:)<0) = 0;
    AtabC(4,AtabC(4,:)<0) = 0;
    AtabC(6,AtabC(6,:)<0) = 0;
    AtabC(7,AtabC(7,:)<0) = 0;
...
%%
writetable(Atab1,[spath,'LMEs_corr_catch_satyrs_driver_obsfish_effort_maxcorr_posfood_A.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Ftab1,[spath,'LMEs_corr_catch_satyrs_driver_obsfish_effort_maxcorr_posfood_F.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Ptab1,[spath,'LMEs_corr_catch_satyrs_driver_obsfish_effort_maxcorr_posfood_P.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(Dtab1,[spath,'LMEs_corr_catch_satyrs_driver_obsfish_effort_maxcorr_posfood_D.csv'],...
    'Delimiter',',','WriteRowNames',true);

save([spath,'LMEs_corr_catch_satyrs_driver_obsfish_effort_maxcorr_posfoods.mat'],...
