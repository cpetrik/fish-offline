% FEISTY output from CESM-4P2Z Hist run
...
%% SP
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'sml_p.nc'],'NC_NOWRITE');
...
%% SF
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'sml_f.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% SD
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'sml_d.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% MP
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'med_p.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% MF
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'med_f.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% MD
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'med_d.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% LP
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'lrg_p.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% LD
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'lrg_d.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
% Benthic material
ncid = netcdf.open([fpath '4P2Z_Hist_' harv 'bent.nc'],'NC_NOWRITE');
[ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
for i = 1:nvars
    varname = netcdf.inqVar(ncid, i-1);
    eval([ varname ' = netcdf.getVar(ncid,i-1);']);
...
SF = NaN*ones(ni,nj,nt);
SP = NaN*ones(ni,nj,nt);
SD = NaN*ones(ni,nj,nt);
MF = NaN*ones(ni,nj,nt);
MP = NaN*ones(ni,nj,nt);
MD = NaN*ones(ni,nj,nt);
LP = NaN*ones(ni,nj,nt);
LD = NaN*ones(ni,nj,nt);
BE = NaN*ones(ni,nj,nt);
...
SF(isnan(SF)) = 1.000000020040877e-20;
SP(isnan(SP)) = 1.000000020040877e-20;
SD(isnan(SD)) = 1.000000020040877e-20;
MF(isnan(MF)) = 1.000000020040877e-20;
MP(isnan(MP)) = 1.000000020040877e-20;
MD(isnan(MD)) = 1.000000020040877e-20;
LP(isnan(LP)) = 1.000000020040877e-20;
LD(isnan(LD)) = 1.000000020040877e-20;
BE(isnan(BE)) = 1.000000020040877e-20;
...
fname1 = 'feisty_cesm-4P2Z_Hist_monthly_';
fname3 = '.nc';

file_sfb = [fpath fname1 'sfb' fname3];
file_spb = [fpath fname1 'spb' fname3];
file_sdb = [fpath fname1 'sdb' fname3];
file_mfb = [fpath fname1 'mfb' fname3];
file_mpb = [fpath fname1 'mpb' fname3];
file_mdb = [fpath fname1 'mdb' fname3];
file_lpb = [fpath fname1 'lpb' fname3];
file_ldb = [fpath fname1 'ldb' fname3];
file_bb  = [fpath fname1 'bb' fname3];
...
%% SF
ncidFB = netcdf.create(file_sfb,cmode);
...
vidbioFB = netcdf.defVar(ncidFB,'sfb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidFB,vidbioFB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidFB,vidbioFB,'long_name','Biomass of Small (Immature) Forage Fish');
...
netcdf.putVar(ncidFB,vidbioFB,SF);
...
%% SP
ncidPB = netcdf.create(file_spb,cmode);
...
vidbioPB = netcdf.defVar(ncidPB,'spb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidPB,vidbioPB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidPB,vidbioPB,'long_name','Biomass of Small (Larval) Large Pelagic Fish');
...
netcdf.putVar(ncidPB,vidbioPB,SP);
netcdf.putVar(ncidPB,vidtPB,yr);

netcdf.close(ncidPB);

%% SD
ncidDB = netcdf.create(file_sdb,cmode);
...
vidbioDB = netcdf.defVar(ncidDB,'sdb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidDB,vidbioDB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidDB,vidbioDB,'long_name','Biomass of Small (Larval) Demersal Fish');
...
netcdf.putVar(ncidDB,vidbioDB,SD);
netcdf.putVar(ncidDB,vidtDB,yr);

netcdf.close(ncidDB);

%% Bent
ncidBB = netcdf.create(file_bb,cmode);
...
vidbioBB = netcdf.defVar(ncidBB,'bb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
...
netcdf.putVar(ncidBB,vidbioBB,BE);
netcdf.putVar(ncidBB,vidtBB,yr);

netcdf.close(ncidBB);

%% MF
ncidSB = netcdf.create(file_mfb,cmode);
...
vidbioSB = netcdf.defVar(ncidSB,'mfb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidSB,vidbioSB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidSB,vidbioSB,'long_name','Biomass of Medium (Adult) Forage Fish');
...
netcdf.putVar(ncidSB,vidbioSB,MF);
netcdf.putVar(ncidSB,vidtSB,yr);

netcdf.close(ncidSB);

%% MP
ncidMB = netcdf.create(file_mpb,cmode);
...
vidbioMB = netcdf.defVar(ncidMB,'mpb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidMB,vidbioMB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidMB,vidbioMB,'long_name','Biomass of Medium (Juvenile) Large Pelagic Fish');
...
netcdf.putVar(ncidMB,vidbioMB,MP);
netcdf.putVar(ncidMB,vidtMB,yr);

netcdf.close(ncidMB);

%% MD
ncidLB = netcdf.create(file_mdb,cmode);
...
vidbioLB = netcdf.defVar(ncidLB,'mdb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidLB,vidbioLB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidLB,vidbioLB,'long_name','Biomass of Medium (Juvenile) Demersal Fish');
...
netcdf.putVar(ncidLB,vidbioLB,MD);
netcdf.putVar(ncidLB,vidtLB,yr);

netcdf.close(ncidLB);

%% LP
ncidVB = netcdf.create(file_lpb,cmode);
...
vidbioVB = netcdf.defVar(ncidVB,'lpb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidVB,vidbioVB,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidVB,vidbioVB,'long_name','Biomass of Large (Adult) Large Pelagic Fish');
...
netcdf.putVar(ncidVB,vidbioVB,LP);
...
%% LD
ncidLD = netcdf.create(file_ldb,cmode);

time_dim = netcdf.defDim(ncidLD,'time',nt);
lon_dim = netcdf.defDim(ncidLD,'nlon',ni);
lat_dim = netcdf.defDim(ncidLD,'nlat',nj);

vidtLD = netcdf.defVar(ncidLD,'time','NC_DOUBLE',time_dim);
netcdf.putAtt(ncidLD,vidtLD,'long_name','time');
netcdf.putAtt(ncidLD,vidtLD,'standard_name','time');
netcdf.putAtt(ncidLD,vidtLD,'calendar','365_day');
netcdf.putAtt(ncidLD,vidtLD,'axis','T');
netcdf.putAtt(ncidLD,vidtLD,'units','year' );

vidlon = netcdf.defVar(ncidLD,'lon','NC_DOUBLE',[lon_dim,lat_dim]);
netcdf.putAtt(ncidLD,vidlon,'long_name','longitude');
netcdf.putAtt(ncidLD,vidlon,'standard_name','longitude');
netcdf.putAtt(ncidLD,vidlon,'units','degrees_east' );
netcdf.putAtt(ncidLD,vidlon,'axis','X');

vidlat = netcdf.defVar(ncidLD,'lat','NC_DOUBLE',[lon_dim,lat_dim]);
netcdf.putAtt(ncidLD,vidlat,'long_name','latitude');
netcdf.putAtt(ncidLD,vidlat,'standard_name','latitude');
netcdf.putAtt(ncidLD,vidlat,'units','degrees_north');
netcdf.putAtt(ncidLD,vidlat,'axis','Y');

vidbioLD = netcdf.defVar(ncidLD,'ldb','NC_FLOAT',[lon_dim,lat_dim,time_dim]);
netcdf.defVarChunking(ncidLD,vidbioLD,'CHUNKED',[10, 10, 1]);
netcdf.putAtt(ncidLD,vidbioLD,'long_name','Biomass of Large (Adult) Demersal Fish');
netcdf.putAtt(ncidLD,vidbioLD,'units','gWW m-2' );
netcdf.defVarFill(ncidLD,vidbioLD,false,1.000000020040877e-20);
netcdf.putAtt(ncidLD,vidbioLD,'missing value',1.000000020040877e-20);

varid = netcdf.getConstant('GLOBAL');
netcdf.putAtt(ncidLD,varid,'creation_date',datestr(now));
netcdf.putAtt(ncidLD,varid,'_FillValue',1.000000020040877e-20);
netcdf.putAtt(ncidLD,varid,'contact','C. Petrik <cpetrik@ucsd.edu>');
netcdf.putAtt(ncidLD,varid,'institution','UCSD');
netcdf.putAtt(ncidLD,varid,'wet weight:C ratio','9:1');

netcdf.endDef(ncidLD);

%tbb = single(tbb);
netcdf.putVar(ncidLD,vidlat,LAT);
netcdf.putVar(ncidLD,vidlon,LON);
netcdf.putVar(ncidLD,vidbioLD,LD);
netcdf.putVar(ncidLD,vidtLD,yr);
