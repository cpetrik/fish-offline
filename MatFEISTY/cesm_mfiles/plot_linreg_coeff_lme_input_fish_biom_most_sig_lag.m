...
load([spath,'LMEs_SLR_biom_driver_maxR2s.mat']) %table of coeff of best driver at best lag
...
for i=1:length(lid)
    L=lid(i);
    if (LAtab(i,2) <= 0.05)
        b=bar(L,LAtab(i,1),'EdgeColor',mcol(LAtab(i,5),:),'FaceColor',mcol(LAtab(i,5),:));
        hold on
    else
        b=bar(L,LAtab(i,1),'EdgeColor',mcol(LAtab(i,5),:),'FaceColor',[1 1 1]);
        hold on
    end
    xtips1 = b(1).XEndPoints;
    ytips1 = b(1).YEndPoints;
    labels = num2str(LAtab(i,4));
    if (LAtab(i,1) < 0)
...
xlabel('LME')
ylabel('Biomass LR Coeff')
title('All fishes')
print('-dpng',[ppath 'Bar_LMEs_driver_biom_SLR_maxR2s_allfish.png'])
...
for i=1:length(lid)
    L=lid(i);
    if (LFtab(i,2) <= 0.05)
        b=bar(L,LFtab(i,1),'EdgeColor',mcol(LFtab(i,5),:),'FaceColor',mcol(LFtab(i,5),:));
        hold on
    else
        b=bar(L,LFtab(i,1),'EdgeColor',mcol(LFtab(i,5),:),'FaceColor',[1 1 1]);
        hold on
    end
    xtips1 = b(1).XEndPoints;
    ytips1 = b(1).YEndPoints;
    labels = num2str(LFtab(i,4));
    if (LFtab(i,1) < 0)
        text(xtips1,ytips1,labels,'HorizontalAlignment','center',...
            'VerticalAlignment','top')
    else
...
for i=1:length(lid)
    L=lid(i);
    if (LPtab(i,2) <= 0.05)
        b=bar(L,LPtab(i,1),'EdgeColor',mcol(LPtab(i,5),:),'FaceColor',mcol(LPtab(i,5),:));
        hold on
    else
        b=bar(L,LPtab(i,1),'EdgeColor',mcol(LPtab(i,5),:),'FaceColor',[1 1 1]);
        hold on
    end
    xtips1 = b(1).XEndPoints;
    ytips1 = b(1).YEndPoints;
    labels = num2str(LPtab(i,4));
    if (LPtab(i,1) < 0)
        text(xtips1,ytips1,labels,'HorizontalAlignment','center',...
            'VerticalAlignment','top')
    else
...
for i=1:length(lid)
    L=lid(i);
    if (LDtab(i,2) <= 0.05)
        b=bar(L,LDtab(i,1),'EdgeColor',mcol(LDtab(i,5),:),'FaceColor',mcol(LDtab(i,5),:));
        hold on
    else
        b=bar(L,LDtab(i,1),'EdgeColor',mcol(LDtab(i,5),:),'FaceColor',[1 1 1]);
        hold on
    end
    xtips1 = b(1).XEndPoints;
    ytips1 = b(1).YEndPoints;
    labels = num2str(LDtab(i,4));
    if (LDtab(i,1) < 0)
        text(xtips1,ytips1,labels,'HorizontalAlignment','center',...
            'VerticalAlignment','top')
    else
...
for i=1:length(lid)
    L=lid(i);
    if (LBtab(i,2) <= 0.05)
        b=bar(L,LBtab(i,1),'EdgeColor',mcol(LBtab(i,5),:),'FaceColor',mcol(LBtab(i,5),:));
        hold on
    else
        b=bar(L,LBtab(i,1),'EdgeColor',mcol(LBtab(i,5),:),'FaceColor',[1 1 1]);
        hold on
    end
    xtips1 = b(1).XEndPoints;
    ytips1 = b(1).YEndPoints;
    labels = num2str(LBtab(i,4));
    if (LBtab(i,1) < 0)
        text(xtips1,ytips1,labels,'HorizontalAlignment','center',...
            'VerticalAlignment','top')
    else
...
print('-dpng',[ppath 'Bar_LMEs_driver_biom_SLR_maxR2s_fntypes.png'])

%% Fn types no benthos
f12 = figure('Units','inches','Position',[1 3 7.5 10]);
subplot('Position',[0.1 0.74 0.856 0.22]) 
% Get fake colors first for legend
for i=1:4
    L=lid(i);
    b=bar(L,LAtab(i,1),'EdgeColor','none','FaceColor',mcol(i,:));
...
subplot('Position',[0.1 0.51 0.7 0.22])
for i=1:length(lid)
    L=lid(i);
    if (LFtab(i,2) <= 0.05)
        b=bar(L,LFtab(i,1),'EdgeColor',mcol(LFtab(i,5),:),'FaceColor',mcol(LFtab(i,5),:));
...
subplot('Position',[0.1 0.28 0.7 0.22]) 
for i=1:length(lid)
    L=lid(i);
    if (LPtab(i,2) <= 0.05)
        b=bar(L,LPtab(i,1),'EdgeColor',mcol(LPtab(i,5),:),'FaceColor',mcol(LPtab(i,5),:));
...
print('-dpng',[ppath 'Bar_LMEs_driver_biom_SLR_maxR2s_fntypes_noBent.png'])
...
for i=1:length(lid)
    ...
    str = {['coef=' sprintf('%0.2f',LAtab(i,1))] , ['lag=' num2str(LAtab(i,4))]};
...
title('Biomass LR All fishes')
stamp('Biomass LR')
print('-dpng',[ppath 'Map_LMEs_driver_biom_SLR_maxR2s_allfish_dominant.png'])
...
for i=1:length(lid)
    ...
    str = {['coef=' sprintf('%0.2f',LFtab(i,1))] , ['lag=' num2str(LFtab(i,4))]};
    %     textm(TLAT(id(1)),TLONG(id(1)),str,...
    %         'HorizontalAlignment','center')
    %     textm(TLAT(id(2)),TLONG(id(2)),['lag=' num2str(LFtab(i,4))],...
    %         'HorizontalAlignment','center')
end
colormap(mcol)
title('Forage fishes')
...
for i=1:length(lid)
    ...
    str = {['coef=' sprintf('%0.2f',LPtab(i,1))] , ['lag=' num2str(LPtab(i,4))]};
end
colormap(mcol)
...
for i=1:length(lid)
    ...
    str = {['coef=' sprintf('%0.2f',LDtab(i,1))] , ['lag=' num2str(LDtab(i,4))]};
...
for i=1:length(lid)
    ...
    str = {['coef=' sprintf('%0.2f',LBtab(i,1))] , ['lag=' num2str(LBtab(i,4))]};
...
title('Benthos')
stamp('Biomass LR')
print('-dpng',[ppath 'Map_LMEs_driver_biom_SLR_maxR2s_fntypes_dominant.png'])
...
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
hold on
surfm(TLAT,TLONG,Acoef)
hold on
colormap(cmRB)
colorbar
clim([-1 1])
title('Biomass LR coeff All fishes')
stamp('Biomass LR')
print('-dpng',[ppath 'Map_LMEs_driver_biom_SLR_maxR2s_allfish_coeff.png'])
...
colorbar
clim([0 0.9])
title('Biomass LR R^2 All fishes')
stamp('Biomass LR')
print('-dpng',[ppath 'Map_LMEs_driver_biom_SLR_maxR2s_allfish_R2.png'])
