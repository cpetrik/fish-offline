...
for L = 1:length(lid)
    ...
    for j = 1:length(tanom)
        ...
        for k=1:length(yr) %Correlations at diff lags
            ...
            %Fish
            [rp,pp] = corrcoef(sclim , (aa_cpue97(i,yst+t:yen))');
            AtabC(L,j,k) = rp(1,2);
            AtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(sclim , (af_cpue97(i,yst+t:yen))');
            FtabC(L,j,k) = rp(1,2);
            FtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(sclim , (ap_cpue97(i,yst+t:yen))');
            PtabC(L,j,k) = rp(1,2);
            PtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(sclim , (ad_cpue97(i,yst+t:yen))');
            DtabC(L,j,k) = rp(1,2);
            DtabP(L,j,k) = pp(1,2);
            clear rp pp
...
%%
save([spath,'LMEs_corr_cpue_chlyrs_driver_lags.mat'],...
