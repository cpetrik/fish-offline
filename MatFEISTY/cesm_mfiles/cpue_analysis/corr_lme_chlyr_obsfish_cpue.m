...
% Biomass
load([fpath 'FEISTY_FOSI_',mod,'_lme_biom_ann_mean_anoms_1997_2010.mat'],'eyr',...
    'aba10','abd10','abf10','abp10');

% Nu
load([fpath 'FEISTY_FOSI_',mod,'_lme_nu_ann_mean_anoms_1997_2010.mat'],...
...
load([ypath 'FishMIP_Phase3a_LME_CPUE_1997-2010_ann_mean_anoms.mat'],...
    'aa_cpue97','af_cpue97','ap_cpue97','ad_cpue97')
...
for L = 1:length(lid)
    ...
    for j = 1:length(tanom)
        ...
        for k=1:length(yr) %Correlations at diff lags
            ...
            %               LME     time      driver
            %Fish
            [rp,pp] = corrcoef(((aanom(i,yst:yen-t,j))') , (aa_cpue97(i,yst+t:yen))');
            AtabC(L,j,k) = rp(1,2);
            AtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(((fanom(i,yst:yen-t,j))') , (af_cpue97(i,yst+t:yen))');
            FtabC(L,j,k) = rp(1,2);
            FtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(((panom(i,yst:yen-t,j))') , (ap_cpue97(i,yst+t:yen))');
            PtabC(L,j,k) = rp(1,2);
            PtabP(L,j,k) = pp(1,2);
            clear rp pp

            [rp,pp] = corrcoef(((danom(i,yst:yen-t,j))') , (ad_cpue97(i,yst+t:yen))');
            DtabC(L,j,k) = rp(1,2);
            DtabP(L,j,k) = pp(1,2);
            clear rp pp
...
%%
save([spath,'LMEs_corr_cpue_chlyrs_obsfish_lags.mat'],...
