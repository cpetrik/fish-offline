...
%% All corrs
CFtab = nan*ones(63,7,5);
PFtab = nan*ones(63,7,5);
CPtab = CFtab;
PPtab = CFtab;
CDtab = CFtab;
PDtab = CFtab;
...
%fish - const effort
CAtab(:,5:7,1:4) = AtabC(:,1:3,:);
CFtab(:,5:7,1:4) = FtabC(:,1:3,:);
CPtab(:,5:7,1:4) = PtabC(:,1:3,:);
CDtab(:,5:7,1:4) = DtabC(:,1:3,:);

PAtab(:,5:7,1:4) = AtabP(:,1:3,:);
PFtab(:,5:7,1:4) = FtabP(:,1:3,:);
PPtab(:,5:7,1:4) = PtabP(:,1:3,:);
PDtab(:,5:7,1:4) = DtabP(:,1:3,:);
...
for j=1:2 %pos & neg w/temp
    ...
    print('-dpng',[ppath 'Map_LMEs_corr_coeff_v3_',tanom{j},'_catch_fntypes_obsfish.png'])
end
...
for j=3:length(ctex)
    ...
    title(['corr Catch All with ' tanom{j}])
    %stamp('')
    print('-dpng',[ppath 'Map_LMEs_pos_corr_coeff_v3_',tanom{j},'_catch_fntypes_obsfish.png'])
    ...
    % title(['corr Catch All with ' tanom{j}])
    % %stamp('')
    % print('-dpng',[ppath 'Map_LMEs_pos_corr_coeff_v3_',tanom{j},'_catch_all_obsfish.png'])
...
for j=5:7
    ...
    %stamp('')
    print('-dpng',[ppath 'Map_LMEs_pos_corr_coeff_v3_',tanom{j},'_catch_fntypes_R2_obsfish.png'])
...
set(gcf,'renderer','painters')
title('Det')
print('-dpng',[ppath 'Map_LMEs_corr_coeff_v3_drivers_posfood_catch_allfish_obsfish.png'])
...
print('-dpng',[ppath 'Map_LMEs_pos_corr_coeff_v3_drivers_catch_allfish_obsfish.png'])
