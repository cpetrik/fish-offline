# Heatmaps of Pearson corrs of single forcing on fish cpue
# Includes 3 lags: 0,1,2
# From (2/7/2024) output
# No zmeso
# Reduce to lag with highest coeff
# 2-4 clusters
...
fcoef <- read.csv(paste0(datar,"LME_sig_corr_maxlag_driver_cpue_F.csv"),sep=",",header = T,stringsAsFactors = F)
pcoef <- read.csv(paste0(datar,"LME_sig_corr_maxlag_driver_cpue_P.csv"),sep=",",header = T,stringsAsFactors = F)
dcoef <- read.csv(paste0(datar,"LME_sig_corr_maxlag_driver_cpue_D.csv"),sep=",",header = T,stringsAsFactors = F)
acoef <- read.csv(paste0(datar,"LME_sig_corr_maxlag_driver_cpue_A.csv"),sep=",",header = T,stringsAsFactors = F)
...
fcoef50 <- fcoef[rowSums(is.na(fcoef)) != 4, ]
pcoef50 <- pcoef[rowSums(is.na(pcoef)) != 4, ]
dcoef50 <- dcoef[rowSums(is.na(dcoef)) != 4, ]
acoef50 <- acoef[rowSums(is.na(acoef)) != 4, ]
...
png(paste0(figp,"heatmap_LME_driver_cpue_A_sig_corr_coeffs_6clus.png"),    # create PNG for the heat map        
...
png(paste0(figp,"heatmap_LME_driver_cpue_F_sig_corr_coeffs_4clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
png(paste0(figp,"heatmap_LME_driver_cpue_P_sig_corr_coeffs_4clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
ddend_7 <- cutree(ddfish, k=5)
dcoef50$Cluster <- ddend_7
# reduce the size of the labels:
ddfish <- set(ddfish, "labels_cex", 0.8)

png(paste0(figp,"heatmap_LME_driver_cpue_D_sig_corr_coeffs_5clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
### Save clustering
write.table(acoef,paste0(datar,"LME_driver_cpue_A_sig_corr_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(fcoef,paste0(datar,"LME_driver_cpue_F_sig_corr_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(pcoef,paste0(datar,"LME_driver_cpue_P_sig_corr_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(dcoef,paste0(datar,"LME_driver_cpue_D_sig_corr_coeffs_cluster.csv"),sep=",",row.names=F)
