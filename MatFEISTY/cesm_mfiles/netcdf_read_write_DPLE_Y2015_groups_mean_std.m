...
allF = nan*ones(nid,nt,length(submem));
allP = allF;
allD = allF;
allB = allF;
% allS = allF;
...
for mem=1%:length(submem) %will loop over
    ...
    % put each ensem member in an array
    allF(:,:,mem) = SF.bio + MF.bio;
    allP(:,:,mem) = SP.bio + MP.bio + LP.bio;
    allD(:,:,mem) = SD.bio + MD.bio + LD.bio;
    allB(:,:,mem) = Bent.bio;
    % allS(:,:,mem) = SF.bio + SP.bio + SD.bio;
    % allM(:,:,mem) = MF.bio + MP.bio + MD.bio;
    % allL(:,:,mem) = LP.bio + LD.bio;
    
    
end

%% take ensemble mean and std dev
