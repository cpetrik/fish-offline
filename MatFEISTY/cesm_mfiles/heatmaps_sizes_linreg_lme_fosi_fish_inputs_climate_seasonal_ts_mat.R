...
ebsNino34 <- ebsNino34[ebsNino34$Type %in% sid,]
...
aiNino34 <- aiNino34[aiNino34$Type %in% sid,]
...
gakPDO <- gakPDO[gakPDO$Type %in% sid,]
...
gakNino34 <- gakNino34[gakNino34$Type %in% sid,]
...
hiNino34 <- hiNino34[hiNino34$Type %in% sid,]
...
chkPDO <- chkPDO[chkPDO$Type %in% sid,]
...
chkAMO <- chkAMO[chkAMO$Type %in% sid,]
...
gmxPDO <- gmxPDO[gmxPDO$Type %in% sid,]
...
gmxNAO <- gmxNAO[gmxNAO$Type %in% sid,]
...
png(paste0(figp,'Heatmaps_CCE_EBS_GAK_NE_coef_sizes_climate_seasonal_inputs_fish.png'), 
...
png(paste0(figp,'Heatmaps_AI_GAK_HI_GMX_SE_coef_sizes_climate_seasonal_inputs_fish.png'), 
    width = 7.5*300,        # 5 x 300 pixels
    height = 10*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)
...
### ALL US LMEs
png(paste0(figp,'Heatmaps_USLMEs_coef_sizes_climate_seasonal_inputs_fish.png'), 
    width = 7.5*300,        # 5 x 300 pixels
    height = 10*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)
plot_grid( c4,c5,'', #ab
           ...
           h4,h5,'', #st
           m5,m3,m1, #uvw
           nrow = 9, ncol = 3,labels = c("a","b","","c","d","e","f","g","h",
                                         "i","j","k","l","m","n","o","p","",
                                         "q","r","","s","t","","u","v","w"), label_size = 11)
dev.off()
