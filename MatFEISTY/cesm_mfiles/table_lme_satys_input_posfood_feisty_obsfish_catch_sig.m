...
%%  ---------------- obsfish --------------------------
load([spath,'LMEs_corr_catch_satyrs_driver_obsfish_maxcorr_posfoods.mat'])

otex = {'TP','TB','Det','ZmLoss','Biom','Prod','Catch','SST','Chl'};
...
matA(1,4) = sum(AtabO(:,4)==8);
matA(2,4) = sum(AtabO(:,4)==9);
...
matF(1,3) = sum(FtabF(:,4)==8);
matF(2,3) = sum(FtabF(:,4)==9);
...
matP(1,4) = sum(PtabO(:,4)==8);
matP(2,4) = sum(PtabO(:,4)==9);
...
matD(1,4) = sum(DtabO(:,4)==8);
matD(2,4) = sum(DtabO(:,4)==9);
...
matD(9,4) = sum(DtabO(:,4)==7);

% I feel like corrs with sat shouldn't incr when adding drivers

%%
lname = cellstr(num2str(lid));
cname = {'Sat','Sat-OBGC','Sat-OBGC-FishCon','Sat-OBGC-FishObs'};
rname = {'SST','Chl','TP','TB','Det','ZmLoss','Biom','Prod','Catch'};
...
%%
writetable(TabA,[spath,'Num_LMEs_corr_catch_satyrs_driver_feisty_maxcorr_posfood_A.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(TabF,[spath,'Num_LMEs_corr_catch_satyrs_driver_feisty_maxcorr_posfood_F.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(TabP,[spath,'Num_LMEs_corr_catch_satyrs_driver_feisty_maxcorr_posfood_P.csv'],...
    'Delimiter',',','WriteRowNames',true);
writetable(TabD,[spath,'Num_LMEs_corr_catch_satyrs_driver_feisty_maxcorr_posfood_D.csv'],...
    'Delimiter',',','WriteRowNames',true);

save([spath,'Num_LMEs_corr_catch_satyrs_driver_feisty_maxcorr_posfoods.mat'],...
    'matF','matP','matD','matA',...
