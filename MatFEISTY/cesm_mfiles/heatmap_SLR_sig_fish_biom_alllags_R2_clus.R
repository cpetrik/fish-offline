...
fcoef <- read.csv(paste0(datar,"LME_sig_SLR_maxR2lag_driver_biom_F.csv"),sep=",",header = T,stringsAsFactors = F)
pcoef <- read.csv(paste0(datar,"LME_sig_SLR_maxR2lag_driver_biom_P.csv"),sep=",",header = T,stringsAsFactors = F)
dcoef <- read.csv(paste0(datar,"LME_sig_SLR_maxR2lag_driver_biom_D.csv"),sep=",",header = T,stringsAsFactors = F)
acoef <- read.csv(paste0(datar,"LME_sig_SLR_maxR2lag_driver_biom_A.csv"),sep=",",header = T,stringsAsFactors = F)
...
adfish <- color_branches(adfish, k=2) 
adend_6 <- cutree(adfish, k=2)
...
png(paste0(figp,"heatmap_LME_driver_biom_A_sig_SLR_coeffs_2clus.png"),    # create PNG for the heat map        
...
png(paste0(figp,"heatmap_LME_driver_biom_F_sig_SLR_coeffs_4clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
png(paste0(figp,"heatmap_LME_driver_biom_P_sig_SLR_coeffs_5clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
# Color the branches based on the clusters:
ddfish <- color_branches(ddfish,k=2) #
ddend_7 <- cutree(ddfish, k=2)
dcoef50$Cluster <- ddend_7
# reduce the size of the labels:
ddfish <- set(ddfish, "labels_cex", 0.8)

png(paste0(figp,"heatmap_LME_driver_biom_D_sig_SLR_coeffs_2clus.png"),    # create PNG for the heat map        
    width = 5*300,        # 5 x 300 pixels
    height = 6.5*300,
    res = 300,            # 300 pixels per inch
    pointsize = 8)  
...
### Save clustering
write.table(acoef,paste0(datar,"LME_driver_biom_A_sig_SLR_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(fcoef,paste0(datar,"LME_driver_biom_F_sig_SLR_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(pcoef,paste0(datar,"LME_driver_biom_P_sig_SLR_coeffs_cluster.csv"),sep=",",row.names=F)
write.table(dcoef,paste0(datar,"LME_driver_biom_D_sig_SLR_coeffs_cluster.csv"),sep=",",row.names=F)
