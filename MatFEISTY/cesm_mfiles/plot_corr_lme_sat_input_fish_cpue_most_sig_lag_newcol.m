...
%% What drives the fish biomass and prod in those LMEs?
load([spath,'LMEs_corr_driver_maxcorrs.mat'],'LAtab')
Btab = LAtab;
clear LAtab
...
%%  ---------------------------------------------------------
itex = {'TP','TB','Det','ZmLoss'};

icol = [238/255 102/255 119/255;... %red - TP
    0/255 68/255 136/255;...    %blue - TB
    34/255 136/255 51/255;...   %green - Det
    51/255 187/255 238/255;...  %cyan - ZL
    ];
...
%%
figure(10)
subplot('Position',[0.01 0.575 0.45 0.45]) %Biom
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
hold on
...
subplot('Position',[0.5 0.575 0.45 0.45]) %Prod
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
hold on
...
% Just LMEs with sig cpue corr - NOT RIGHT
bid = find(Atab(:,4)==5);
pid = find(Atab(:,4)==6);
sid = [bid;pid];
...
subplot('Position',[0.01 0.05 0.45 0.45]) %Biom
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
hold on
...
subplot('Position',[0.5 0.05 0.45 0.45]) %Prod
axesm ('Robinson','MapLatLimit',clatlim,'MapLonLimit',clonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
hold on
...
print('-dpng',[ppath 'Map_LMEs_driver_biom_prod_maxcorr_allfish.png'])
