...
load([fpath 'Time_Means_HR_' mod '_' cfile '.mat']);
load([fpath 'Space_Means_HR_' mod '_' cfile '.mat']);
load([fpath 'Annual_Means_HR_' mod '_' cfile '.mat'],'mz_mtf');

%% Map data
cpath='/Volumes/MIP/GCM_DATA/CESM_HR/';
load([cpath 'Data_grid_g.e22.G1850ECO_JRA_HR.TL319_t13.004.CAcurr.mat'],'GRD');
load([cpath 'g.e22.G1850ECO_JRA_HR.TL319_t13.004.grid.mat']);

TLONG = squeeze(TLONG(:,:,1));
TLAT = squeeze(TLAT(:,:,1));
[ni,nj]=size(TLONG);

plotminlat=25; %Set these bounds for your data
plotmaxlat=55;
% plotminlon=-180;
% plotmaxlon=180;
plotminlon=-145;
...
title('HR')
stamp(mod)
print('-dpng',[ppath 'HR_',mod,'_all_sizes.png'])
...
title('HR')
stamp(mod)
print('-dpng',[ppath 'HR_',mod,'_all_types.png'])
...
save([fpath 'Plot_Means_HR_' mod '_' cfile '.mat'],'F','P','D','B',...
...
title('HR log10 mean benthic biomass (g m^-^2)')
stamp('')
print('-dpng',[ppath 'HR_',mod,'_global_BENT.png'])

%% All 4 on subplots
figure(4)
% all F
subplot('Position',[0 0.51 0.5 0.45])
axesm ('Robinson','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
...
%h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
caxis([-2 2]);
colorbar('Position',[0.5 0.25 0.03 0.5],'orientation','vertical')
set(gcf,'renderer','painters')
...
% All P
subplot('Position',[0.5 0.51 0.5 0.45])
axesm ('Robinson','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
...
stamp('')
print('-dpng',[ppath 'HR_',mod,'_global_All_subplot.png'])

%% Ratios on subplots red-white-blue
% 3 figure subplot P:D, P:F, M:L
figure(5)
subplot('Position',[0 0.52 0.5 0.45])
%P:D
axesm ('Robinson','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
...
%P:F
subplot('Position',[0.5 0.52 0.5 0.45])
axesm ('Robinson','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
...
%L:M
subplot('Position',[0.25 0.0 0.5 0.45])
axesm ('Robinson','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
    'Grid','off','FLineWidth',1)
...
%h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
caxis([0 1]);
colorbar('Position',[0.2 0.485 0.6 0.03],'orientation','horizontal')
set(gcf,'renderer','painters')
title('Fraction Large vs. Medium')
stamp('')
print('-dpng',[ppath 'HR_',mod,'_global_ratios_subplot.png'])
...
print('-dpng',[ppath 'HR_',mod '_timeseries_zoop_overcon.png'])
...
% save([bpath 'HR_' mod '_ts_map_zoop_overcon.mat'],...
...
print('-dpng',[ppath 'HR_',mod '_global_zoop_overcon.png'])
