%%%% File naming system
function [fname,simname] = sub_fname_spin_1meso(param,model,exper)
...
if (param.CC==0)
    simname = [coup,'_Lam',tlam(2:end),'_enc',tefn,'-b',tbenc(2:end),'_m',tmfn,'-b',tbfn(2:end),'-k',tkfn(2:end),'_c',tcfn,'-b',tbcmx(2:end),'_D',td(2:end),'_A',ta(2:end),'_sMZ',tmzp(2:end),'_mMZ',tmzp2(2:end),'_nmort',tmort,'_BE',tbe(2:end),'_noCC_RE',tre(2:end)];
else
...
if (~isfolder(['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model]))
    mkdir(['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model])
end

%! Setup netcdf path to store to
if (frate==0)
    fname = ['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model,'/',exper,'_Spinup_pristine'];
elseif (param.Jsel~=0.1)
    fname = ['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model,'/',exper,'_Spinup_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = ['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model,'/',exper,'_Spinup_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/',model,'/',exper,'_Spinup_', sel,'_fish',tfish(2:end)];
end
