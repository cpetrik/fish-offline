%%%% File naming system
function [fname,simname,harv,dirname] = sub_fname_cesm_fosi_exper(param,exper)
...
if (~isfolder(['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/FOSI']))
    mkdir(['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/FOSI'])
end
dirname = ['/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/',simname,'/FOSI'];

%! Setup netcdf path to store to
if (frate==0)
    fname = [dirname, '/FOSI_',exper,'pristine'];
    harv = 'pristine';
elseif (isnan(frate))
    fname = [dirname, '/FOSI_',exper,'obsfish'];
    harv = 'obsfish';
elseif (param.Jsel~=0.1)
    fname = [sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
    harv = [sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = [dirname, '/FOSI_',exper,'fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
    harv = ['fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = [dirname, '/FOSI_',exper,sel,'_fish',tfish(2:end)];  
    harv = [sel,'_fish',tfish(2:end)];
end
