...
% 5. Run FEISTY DPLE with observe fishing effort
% Stop loop
...
Cdir = '/glade/u/home/ekim/feisty/input_files/';
Indir = '/glade/u/home/ekim/feisty/input_files/FOSI_input_file/';
spath = '/glade/u/home/ekim/feisty/dailies/';
...
% DPLE is 1954-2017
% leadtimes = 1:24;
firstyear = 1955; %of DPLE initializations
lastyear  = 2010; %of DPLE initializations
...
%load(['/Volumes/petrik-lab/Feisty/GCM_Data/CESM/FOSI/FOSI_POP_gx1v6_noSeas_fmort_ID_annual_1948_2010_tempSc_assessment.mat'],...
%    'fmD','fmP','fmF');

load([Cdir 'FOSI_POP_gx1v6_noSeas_fmort_ID_annual_1948_2010_tempSc_assessment.mat'],...
    'fmD','fmP','fmF','year');

eff_yr = year;
clear year
eyid = find(eff_yr == StartYr(1));
fmF = fmF(:,eyid:end); %changed!
fmP = fmP(:,eyid:end); %changed!
fmD = fmD(:,eyid:end); %changed!
...
fprintf('The length of Y is %d.\n', length(Y));

for yr=2 %:54 %length(Y) %Y(62)

    fprintf('Current yr: %d\n', yr);  % 디버깅용 출력

	param.frate = nan; %changed!
    param.frateF = fmF(:,yr);
    param.frateP = fmP(:,yr);
    param.frateD = fmD(:,yr);
    param.dfrateF = param.frateF/365.0;
    ...
    for mem=1:length(M)
        ...
        %! Dims of netcdf file
        nt = 2+(12*YEARS); %Nov, Dec, plus full 12-mo years
        ...
        [TEMP_150m,TEMP_bottom,POC_FLUX_IN_bottom,LzooC_150m,Lzoo_loss_150m] = dple_anom_to_raw(fpath,im,yr,ni,nj,L,...
                                                                                clim_Tp,clim_Tb,clim_POC,clim_zooC,clim_loss);
        ...
        for y = 1:nyrs
            ...
            if y==1
                %! Initialize FEISTY
                % FEISTY INIT IS now OCT
                % this will have to be the biomass from the last mo before the start year
                % from the FOSI
                init_sim = ['v15_obsfish_' simname];
                load([Indir,'Last_mo_FOSI_' num2str(StartYr(yr)) '_' init_sim '.mat']);
                BENT.mass = BENT.bio;
