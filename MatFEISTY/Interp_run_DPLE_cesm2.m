...
%%
Cdir = '/Volumes/petrik-lab/Feisty/GCM_DATA/CESM/FOSI/';
fpath = '/Volumes/petrik-lab/Feisty/GCM_Data/CESM/DPLE/';
spath = '/Volumes/petrik-lab/Feisty/NC/CESM_MAPP/';
...
for yr= 40 %41:49 %1:length(Y) %Y(62)
    ...
    for mem=23:length(M)
        ...
        [fname,simname] = sub_fname_dple_exper_server(param,exper);
        ...
        %% 3. Convert DPLE from anom to raw ----------------------------
        [TEMP_150m,TEMP_bottom,POC_FLUX_IN_bottom,LzooC_150m,...
            Lzoo_loss_150m] = dple_anom_to_raw(fpath,im,yr,ni,nj,L,clim_Tp,clim_Tb,clim_POC,clim_zooC,clim_loss);
        ...
        for y = 1:nyrs
            ...
            if y==1
                %! Initialize FEISTY
                % this will have to be the biomass from the Oct before the start year
                % from the FOSI (Or Nov?)
                init_sim = ['v15_All_fish03_' simname];
                load([spath,simname,'/Last_mo_FOSI_Oct_' num2str(StartYr(yr)) '_' init_sim '.mat']);
                BENT.mass = BENT.bio;
