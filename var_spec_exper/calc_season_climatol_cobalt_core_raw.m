...
for m = 1:12
    ...
    lz_clim(:,:,m) = nanmean((lz(:,:,mo)),3);
    mhp_clim(:,:,m) = nanmean((mz_hp(:,:,mo)),3);
    lhp_clim(:,:,m) = nanmean((lz_hp(:,:,mo)),3);
...
% Save climatologies
save([fpath 'cobalt_core_climatol_1950_2007_raw.mat'],...
...
for m = 1:12
    ...
    mz_anom(:,:,mo) = (mz_100(:,:,mo)) - mz_clim(:,:,m);
    lz_anom(:,:,mo) = (lz_100(:,:,mo)) - lz_clim(:,:,m);
    det_anom(:,:,mo) = (det_btm(:,:,mo)) - det_clim(:,:,m);
    hpmz_anom(:,:,mo) = (hploss_mz_100(:,:,mo)) - mhp_clim(:,:,m);
    hplz_anom(:,:,mo) = (hploss_lz_100(:,:,mo)) - lhp_clim(:,:,m);
    
end


%% Save anom
save([fpath 'cobalt_core_anom_1950_2007_raw.mat'],...
    'units_vint','lat','lon','yid',...
