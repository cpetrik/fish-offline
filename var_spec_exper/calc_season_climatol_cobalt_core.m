...
%     det_clim(:,:,m) = nanmean(log10(det(:,:,mo)),3);
    mz_clim(:,:,m) = nanmean(sqrt(mz(:,:,mo)),3);
    lz_clim(:,:,m) = nanmean(sqrt(lz(:,:,mo)),3);
    mhp_clim(:,:,m) = nanmean(sqrt(mz_hp(:,:,mo)),3);
    lhp_clim(:,:,m) = nanmean(sqrt(lz_hp(:,:,mo)),3);
    det_clim(:,:,m) = nanmean(sqrt(det(:,:,mo)),3);
end
...
%% Save climatologies
save([fpath 'cobalt_core_climatol_1950_2007.mat'],...
...
%     hplz_anom(:,:,mo) = log10(hploss_lz_100(:,:,mo)) - lhp_clim(:,:,m);
    mz_anom(:,:,mo) = sqrt(mz_100(:,:,mo)) - mz_clim(:,:,m);
    lz_anom(:,:,mo) = sqrt(lz_100(:,:,mo)) - lz_clim(:,:,m);
    det_anom(:,:,mo) = sqrt(det_btm(:,:,mo)) - det_clim(:,:,m);
    hpmz_anom(:,:,mo) = sqrt(hploss_mz_100(:,:,mo)) - mhp_clim(:,:,m);
    hplz_anom(:,:,mo) = sqrt(hploss_lz_100(:,:,mo)) - lhp_clim(:,:,m);
...
%% Save anom
save([fpath 'cobalt_core_anom_1950_2007.mat'],...
    'units_vint','lat','lon','yid',...
