...
simulationyears = 150;
mz_clim_spl_rep=repmat(mz_clim_spl',[simulationyears,1]);
mhp_clim_spl_rep=repmat(mhp_clim_spl',[simulationyears,1]);

mz_sc_spl_rep=repmat(mz_sc_spl',[simulationyears,1]);
mhp_sc_spl_rep=repmat(mhp_sc_spl',[simulationyears,1]);
...
for b=1:8
    scaled = noise .* mz_sc_spl_rep(:,b)';
    noise_sc_mz(st(b):en(b),:) = scaled;
    
    scaled = noise .* mhp_sc_spl_rep(:,b)';
    noise_sc_mhp(st(b):en(b),:) = scaled;
end

%% Combine repeating climatology and noise, and square
mz_clim_spl_rep_backtrans=(noise_sc_mz + repelem(mz_clim_spl_rep',220,1)).^2;
mhp_clim_spl_rep_backtrans=(noise_sc_mhp + repelem(mhp_clim_spl_rep',220,1)).^2;

save([fpath 'cobalt_biome_core_noise_mz_mhp.mat'],'mz_clim_spl_rep_backtrans',...
    'mhp_clim_spl_rep_backtrans');
