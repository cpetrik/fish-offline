...
for m = 1:12
    mo = m:12:nyr;
    sf_clim(:,m) = nanmean(sqrt(SFb(:,mo)),2);
    sp_clim(:,m) = nanmean(sqrt(SPb(:,mo)),2);
    sd_clim(:,m) = nanmean(sqrt(SDb(:,mo)),2);
    mf_clim(:,m) = nanmean(sqrt(MFb(:,mo)),2);
    mp_clim(:,m) = nanmean(sqrt(MPb(:,mo)),2);
    md_clim(:,m) = nanmean(sqrt(MDb(:,mo)),2);
    lp_clim(:,m) = nanmean(sqrt(LPb(:,mo)),2);
    ld_clim(:,m) = nanmean(sqrt(LDb(:,mo)),2);
    B_clim(:,m)  = nanmean(sqrt(Bb(:,mo)),2);
    F_clim(:,m)  = nanmean(sqrt(Fb(:,mo)),2);
    P_clim(:,m)  = nanmean(sqrt(Pb(:,mo)),2);
    D_clim(:,m)  = nanmean(sqrt(Db(:,mo)),2);
    S_clim(:,m)  = nanmean(sqrt(Sb(:,mo)),2);
    M_clim(:,m)  = nanmean(sqrt(Mb(:,mo)),2);
    L_clim(:,m)  = nanmean(sqrt(Lb(:,mo)),2);
    all_clim(:,m) = nanmean(sqrt(Ab(:,mo)),2);
 end

%% Save climatologies
save([fpath 'feisty_climatol_last50_sqrt.mat'],...
...
for m = 1:12
    mo = m:12:nmo;
    sf_anom(:,mo) = sqrt(SFb(:,mo)) - sf_clim(:,m);
    sp_anom(:,mo) = sqrt(SPb(:,mo)) - sp_clim(:,m);
    sd_anom(:,mo) = sqrt(SDb(:,mo)) - sd_clim(:,m);
    mf_anom(:,mo) = sqrt(MFb(:,mo)) - mf_clim(:,m);
    mp_anom(:,mo) = sqrt(MPb(:,mo)) - mp_clim(:,m);
    md_anom(:,mo) = sqrt(MDb(:,mo)) - md_clim(:,m);
    lp_anom(:,mo) = sqrt(LPb(:,mo)) - lp_clim(:,m);
    ld_anom(:,mo) = sqrt(LDb(:,mo)) - ld_clim(:,m);
    B_anom(:,mo)  = sqrt(Bb(:,mo)) - B_clim(:,m);
    F_anom(:,mo)  = sqrt(Fb(:,mo))  - F_clim(:,m);
    P_anom(:,mo)  = sqrt(Pb(:,mo))  - P_clim(:,m);
    D_anom(:,mo)  = sqrt(Db(:,mo))  - D_clim(:,m);
    S_anom(:,mo)  = sqrt(Sb(:,mo))  - S_clim(:,m);
    M_anom(:,mo)  = sqrt(Mb(:,mo))  - M_clim(:,m);
    L_anom(:,mo)  = sqrt(Lb(:,mo))  - L_clim(:,m);
    all_anom(:,mo) = sqrt(Ab(:,mo)) - all_clim(:,m);
 end

%% Save anom
save([fpath 'feisty_anom_last50_sqrt.mat'],...
...
figure
plot(l50,F_anom(1500,:))

figure
plot(l50,P_anom(1500,:))
