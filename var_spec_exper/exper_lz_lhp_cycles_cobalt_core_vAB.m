...
simulationyears = 150;
lz_clim_spl_rep=repmat(lz_clim_spl',[simulationyears,1]);
lhp_clim_spl_rep=repmat(lhp_clim_spl',[simulationyears,1]);

lz_sc_spl_rep=repmat(lz_sc_spl',[simulationyears,1]);
lhp_sc_spl_rep=repmat(lhp_sc_spl',[simulationyears,1]);
...
for b=1:8
    scaled = noise .* lz_sc_spl_rep(:,b)';
    noise_sc_lz(st(b):en(b),:) = scaled;
    
    scaled = noise .* lhp_sc_spl_rep(:,b)';
    noise_sc_lhp(st(b):en(b),:) = scaled;
end

%% Combine repeating climatology and noise, and square
lz_clim_spl_rep_backtrans=(noise_sc_lz + repelem(lz_clim_spl_rep',220,1)).^2;
lhp_clim_spl_rep_backtrans=(noise_sc_lhp + repelem(lhp_clim_spl_rep',220,1)).^2;

save([fpath 'cobalt_biome_core_noise_lz_lhp.mat'],'lz_clim_spl_rep_backtrans',...
   'lhp_clim_spl_rep_backtrans');
