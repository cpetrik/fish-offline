...
% 
n=L;
xdft=Y(1:n/2);  %Take only the positive half
freq=((0:n/2-1)/n)';
% plot(freq,xdft)
% title('Single-Sided Amplitude Spectrum of X(t)')
% xlabel('f (Hz)')
...
%0-2 yrs
y02 = (yr<2);
D02 = sum(P1(y02)) / sum(P1);

%2-5 yrs
y25 = (yr>=2 & yr<5);
%D25 = sum(Y(y25)) / sum(Y)
D25 = sum(P1(y25)) / sum(P1);

%5-12 yrs
y512 = (yr>=5 & yr<12);
D512 = sum(P1(y512)) / sum(P1);

%12-20 yrs
y1220 = (yr>=12 & yr<20);
D1220 = sum(P1(y1220)) / sum(P1);

%20-30 yrs
y2030 = (yr>=20 & yr<30);
D2030 = sum(P1(y2030)) / sum(P1);

%30-50 yrs
y3050 = (yr>=30 & yr<50);
D3050 = sum(P1(y3050)) / sum(P1);
