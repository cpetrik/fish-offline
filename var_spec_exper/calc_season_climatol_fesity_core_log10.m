...
for m = 1:12
    mo = m:12:nyr;
    sf_clim(:,m) = nanmean(log10(SF.bio(:,mo)),2);
    sp_clim(:,m) = nanmean(log10(SP.bio(:,mo)),2);
    sd_clim(:,m) = nanmean(log10(SD.bio(:,mo)),2);
    mf_clim(:,m) = nanmean(log10(MF.bio(:,mo)),2);
    mp_clim(:,m) = nanmean(log10(MP.bio(:,mo)),2);
    md_clim(:,m) = nanmean(log10(MD.bio(:,mo)),2);
    lp_clim(:,m) = nanmean(log10(LP.bio(:,mo)),2);
    ld_clim(:,m) = nanmean(log10(LD.bio(:,mo)),2);
    B_clim(:,m)  = nanmean(log10(Bent.bio(:,mo)),2);
...
%% Save climatologies
save([fpath 'fesity_core_climatol_1950_2007_log10.mat'],...
...
for m = 1:12
    mo = m:12:nmo;
    sf_anom(:,mo) = log10(SF.bio(:,mo)) - sf_clim(:,m);
    sp_anom(:,mo) = log10(SP.bio(:,mo)) - sp_clim(:,m);
    sd_anom(:,mo) = log10(SD.bio(:,mo)) - sd_clim(:,m);
    mf_anom(:,mo) = log10(MF.bio(:,mo)) - mf_clim(:,m);
    mp_anom(:,mo) = log10(MP.bio(:,mo)) - mp_clim(:,m);
    md_anom(:,mo) = log10(MD.bio(:,mo)) - md_clim(:,m);
    lp_anom(:,mo) = log10(LP.bio(:,mo)) - lp_clim(:,m);
    ld_anom(:,mo) = log10(LD.bio(:,mo)) - ld_clim(:,m);
    B_anom(:,mo)  = log10(Bent.bio(:,mo)) - B_clim(:,m);
...
%% Save anom
save([fpath 'feisty_core_anom_1950_2007_log10.mat'],...
    'geolat_t','geolon_t','yid','GRD',...
