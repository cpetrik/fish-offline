...
for m = 1:12
    mo = m:12:nyr;
    sf_clim(:,m) = nanmean(log(SFb(:,mo)),2);
    sp_clim(:,m) = nanmean(log(SPb(:,mo)),2);
    sd_clim(:,m) = nanmean(log(SDb(:,mo)),2);
    mf_clim(:,m) = nanmean(log(MFb(:,mo)),2);
    mp_clim(:,m) = nanmean(log(MPb(:,mo)),2);
    md_clim(:,m) = nanmean(log(MDb(:,mo)),2);
    lp_clim(:,m) = nanmean(log(LPb(:,mo)),2);
    ld_clim(:,m) = nanmean(log(LDb(:,mo)),2);
    B_clim(:,m)  = nanmean(log(Bb(:,mo)),2);
    F_clim(:,m)  = nanmean(log(Fb(:,mo)),2);
    P_clim(:,m)  = nanmean(log(Pb(:,mo)),2);
    D_clim(:,m)  = nanmean(log(Db(:,mo)),2);
    S_clim(:,m)  = nanmean(log(Sb(:,mo)),2);
    M_clim(:,m)  = nanmean(log(Mb(:,mo)),2);
    L_clim(:,m)  = nanmean(log(Lb(:,mo)),2);
    all_clim(:,m) = nanmean(log(Ab(:,mo)),2);
 end

%% Save climatologies
save([fpath 'feisty_climatol_last50_ln.mat'],...
...
for m = 1:12
    mo = m:12:nmo;
    sf_anom(:,mo) = log(SFb(:,mo)) - sf_clim(:,m);
    sp_anom(:,mo) = log(SPb(:,mo)) - sp_clim(:,m);
    sd_anom(:,mo) = log(SDb(:,mo)) - sd_clim(:,m);
    mf_anom(:,mo) = log(MFb(:,mo)) - mf_clim(:,m);
    mp_anom(:,mo) = log(MPb(:,mo)) - mp_clim(:,m);
    md_anom(:,mo) = log(MDb(:,mo)) - md_clim(:,m);
    lp_anom(:,mo) = log(LPb(:,mo)) - lp_clim(:,m);
    ld_anom(:,mo) = log(LDb(:,mo)) - ld_clim(:,m);
    B_anom(:,mo)  = log(Bb(:,mo)) - B_clim(:,m);
    F_anom(:,mo)  = log(Fb(:,mo))  - F_clim(:,m);
    P_anom(:,mo)  = log(Pb(:,mo))  - P_clim(:,m);
    D_anom(:,mo)  = log(Db(:,mo))  - D_clim(:,m);
    S_anom(:,mo)  = log(Sb(:,mo))  - S_clim(:,m);
    M_anom(:,mo)  = log(Mb(:,mo))  - M_clim(:,m);
    L_anom(:,mo)  = log(Lb(:,mo))  - L_clim(:,m);
    all_anom(:,mo) = log(Ab(:,mo)) - all_clim(:,m);
 end

%% Save anom
save([fpath 'feisty_anom_last50_ln.mat'],...
