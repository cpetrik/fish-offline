...
%% Save climatologies
save([fpath 'fesity_core_climatol_1950_2007_ln.mat'],...
...
for m = 1:12
    mo = m:12:nmo;
    sf_anom(:,mo) = log(SF.bio(:,mo)) - sf_clim(:,m);
    sp_anom(:,mo) = log(SP.bio(:,mo)) - sp_clim(:,m);
    sd_anom(:,mo) = log(SD.bio(:,mo)) - sd_clim(:,m);
    mf_anom(:,mo) = log(MF.bio(:,mo)) - mf_clim(:,m);
    mp_anom(:,mo) = log(MP.bio(:,mo)) - mp_clim(:,m);
    md_anom(:,mo) = log(MD.bio(:,mo)) - md_clim(:,m);
    lp_anom(:,mo) = log(LP.bio(:,mo)) - lp_clim(:,m);
    ld_anom(:,mo) = log(LD.bio(:,mo)) - ld_clim(:,m);
    B_anom(:,mo)  = log(Bent.bio(:,mo)) - B_clim(:,m);
...
%% Save anom
save([fpath 'feisty_core_anom_1950_2007_ln.mat'],...
    'geolat_t','geolon_t','yid','GRD',...
